                    ...
                    <form action="{{ route('wiki.update', $page->slug) }}" method="post" class="wiki-form">
                        ...
                        <div class="mb-3">
                            ...
                            <div class="border rounded-3 p-3 bg-light @error('role_ids') border-danger @enderror" style="max-height: 220px; overflow-y: auto;">
                                @php
                                    $selectedRoleIds = array_map('intval', (array) old('role_ids', $page->roles->pluck('id')->all()));
                                @endphp
                                @forelse($allRoles as $role)
                                    <div class="form-check">
                                        <input class="form-check-input" type="checkbox" name="role_ids[]" value="{{ $role->id }}" id="wiki_role_edit_{{ $role->id }}"
                                            @checked(in_array((int) $role->id, $selectedRoleIds, true))>
                                        <label class="form-check-label" for="wiki_role_edit_{{ $role->id }}">{{ $role->name }}</label>
                                    </div>
                                @empty
