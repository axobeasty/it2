...
<head>
    ...
    <title>{{ $settings->title }} — Расписание преподавателя</title>
    ...
    <style>
        ...
        .lesson-pill { border-left: 4px solid #198754; background: #f8fafc; border-radius: 0 10px 10px 0; padding: .75rem 1rem; margin-bottom: .6rem; }
...
<div class="container py-4">
    ...
    <div class="d-flex flex-wrap align-items-center justify-content-between gap-3 mb-4">
        <div>
            <h4 class="mb-1">Расписание преподавателя</h4>
            <p class="text-muted mb-0">{{ $employee->fio }} · неделя с {{ $weekMonday->format('d.m.Y') }}</p>
        </div>
        <div class="week-nav d-flex align-items-center gap-2">
            ...
            @endphp
            <a class="btn btn-outline-secondary btn-sm" href="{{ route('schedule.teacher', ['week' => $prev]) }}"><i class="bi bi-chevron-left"></i></a>
            <span class="small text-muted px-2">{{ $weekMonday->format('d.m.Y') }} — {{ $weekMonday->copy()->addDays(6)->format('d.m.Y') }}</span>
            <a class="btn btn-outline-secondary btn-sm" href="{{ route('schedule.teacher', ['week' => $next]) }}"><i class="bi bi-chevron-right"></i></a>
            <a class="btn btn-outline-primary btn-sm" href="{{ route('schedule.teacher') }}">Текущая</a>
        </div>
    </div>
                        ...
                        @forelse($dayLessons as $entry)
                            <div class="lesson-pill">
                                ...
                                <div class="small text-muted">
                                    ...
                                    {{ optional($entry->group)->name ?? 'Группа' }}
                                </div>
