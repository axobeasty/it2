...
<head>
    ...
    <title>{{ $settings->title }} — Редактирование теста</title>
...
<div class="container py-4">
    <div class="bg-white rounded shadow-sm p-4 mb-4">
        <div class="d-flex justify-content-between align-items-center mb-3">
            ...
            <a href="/tests/admin" class="btn btn-outline-secondary btn-sm">К списку тестов</a>
        </div>
        <hr>
        <form action="/tests/admin/{{ $test->id }}/update" method="post" id="testBuilderForm">
            @csrf
            <div class="row">
                ...
                <div class="col-md-3 mb-3">
                    ...
                    <input type="number" name="time_limit_minutes" class="form-control" min="1" max="480" value="{{ $test->time_limit_minutes }}">
                </div>
                <div class="col-md-3 mb-3">
                    <label class="form-label">Лимит попыток</label>
                    <input type="number" name="attempts_limit" class="form-control" min="1" max="20" placeholder="Пусто = без лимита" value="{{ $test->attempts_limit }}">
                </div>
                <div class="col-md-12 mb-3 d-flex align-items-end">
                    <div class="form-check form-switch">
                        <input type="hidden" name="is_active" value="0">
                        <input class="form-check-input" type="checkbox" name="is_active" id="isActive" value="1" @if($test->is_active) checked @endif>
            ...
            <div class="mb-4">
                ...
                <select name="group_ids[]" class="form-select" multiple required style="min-height:120px;">
                    @php $assignedIds = $test->assignments->pluck('group_id')->all(); @endphp
                    @foreach($groups as $group)
                        <option value="{{ $group->id }}" @if(in_array((int)$group->id, array_map('intval', $assignedIds), true)) selected @endif>{{ $group->name }}</option>
                    @endforeach
                </select>
            </div>
            ...
            <button type="submit" class="btn btn-dark mt-3">Сохранить изменения</button>
        </form>
    </div>
</div>
...
<script>window.INITIAL_QUESTIONS = @json($builderQuestions ?? []);</script>
...
<script>
    ...
    function addQuestionFromData(data) {
        ...
        card.querySelector('.q-points').value = data.points || 1;
        renderQuestionBody(card);
        const type = data.type;
        if (type === 'single' || type === 'multiple') {
            card.querySelector('.q-options').value = (data.options || []).join('\n');
            card.querySelector('.q-correct').value = data.correct || '';
        } else if (type === 'match') {
            card.querySelector('.q-left').value = (data.left || []).join('\n');
            card.querySelector('.q-right').value = (data.right || []).join('\n');
            card.querySelector('.q-pairs').value = data.pairs_lines || '';
        } else {
            card.querySelector('.q-accepted').value = data.accepted || '';
        }
        questionsContainer.appendChild(fragment);
        refreshIndexes();
    }
    ...
    (function init() {
        ...
        const initial = window.INITIAL_QUESTIONS || [];
        if (initial.length) {
            initial.forEach(addQuestionFromData);
        } else {
            addQuestion();
        }
    })();
